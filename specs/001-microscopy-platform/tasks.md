...
- [X] T157c [US4] Implement PTP camera settings control with proper gphoto2 API usage: use gp_camera_get_single_config/gp_camera_set_single_config for efficient individual setting changes (recommended gphoto2 pattern from source code analysis), fallback to full config tree if single config not supported, add 100ms live view refresh after setting changes to apply updates to preview
- [X] T157d [US4] Implement cross-manufacturer settings support: add conversion functions for numeric ↔ readable values (convertShutterSpeedToReadable/ToNumeric, convertWhiteBalanceToReadable/ToNumeric), detect Sony numeric encoding (d6cc for exposure mode, d6ca for shutter speed, d6c5 for white balance) vs Canon/Nikon/Fuji text values, cache manufacturer-specific widget names (_exposureModeWidgetName, _shutterSpeedWidgetName, _whiteBalanceWidgetName) for efficient access
- [X] T157e [US4] Implement PTP settings persistence and restoration: add getPTPSetting() to read current camera values, savePTPSetting() to persist to QSettings under "camera/<cameraId>/ptp/<settingname>", restorePTPSettings() to restore on camera connection with 500ms delay, connect PTPCameraService::cameraConnected signal to trigger restoration, settings restored: exposuremode, iso, shutterspeed, exposurecompensation, whitebalance
- [X] T157f [US4] Replace aperture control with exposure compensation for microscopy: update CameraControlsPanel to use exposure compensation instead of aperture (fixed depth-of-field in microscopy), hide exposure compensation in Manual mode (user controls shutter directly), show in Aperture Priority mode, rename _ptpApertureCombo to _ptpExposureCompCombo throughout UI
- [X] T157g [US4] Implement live PTP settings UI with current value selection: update CameraControlsPanel::updateControlsVisibility() to read current values from camera after populating combo boxes, select current values in combo boxes, update combos when user changes settings, verify settings visibly affect live view image
- [X] T157h [US4] Integrate PTP image capture into CameraController: update CameraController::captureImage() to detect camera type, for PTP cameras call PTPCameraService::captureImage() which uses gp_camera_capture() to trigger full-resolution capture on camera and transfers via USB (not from live view), for V4L2 cameras use existing captureFrame() from live view, emit imageCaptured signal with saved file path for both types
